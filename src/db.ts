...
// Falls back to localhost postgres for local dev if not set.
// ─────────────────────────────────────────────────────────────

const DATABASE_URL = process.env.DATABASE_URL;

if (!DATABASE_URL) {
  console.warn('  WARNING: DATABASE_URL not set — persistence disabled (simulations will not be saved)');
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const neonFn: any = DATABASE_URL ? neon(DATABASE_URL) : null;
const sql = neonFn;

// ─────────────────────────────────────────────────────────────
// Schema init — runs once on startup (no-op if already created)
// ─────────────────────────────────────────────────────────────

export async function initSchema(): Promise<void> {
  if (!sql) {
    console.log('  Neon: disabled (no DATABASE_URL)');
    return;
  }
...
export async function deleteSimulation(id: string): Promise<void> {
  if (!sql) return;
  // CASCADE handles tributes/events
